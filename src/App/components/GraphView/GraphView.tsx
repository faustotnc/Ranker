...
const GraphView: React.FC<GraphViewProps> = (props: GraphViewProps) => {
    ...
    */
   useEffect(() => {
      let graph = new NetworkGraph(cyContainer.current);
      graph.addNetwork(network.getEdges(), doPowerIteration(network)).rerun();
   }, []);
   ...
   return (
      <Box className="graphView" height="100%" sx={{ display: "flex", flexDirection: "column" }}>
         ...
         <Paper
            variant="outlined"
            elevation={0}
            sx={{ borderRadius: 0, borderBottom: 0, borderLeft: 0, borderRight: 0 }}
            className="node-info-bar"
         >
            <NodeInfo></NodeInfo>
         </Paper>
      </Box>
